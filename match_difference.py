...
for index in range(divide):
    img = cv2.imread("output/{0}/{2}/img/{0}-{1}-{2}.png".format(folder,index,shift),cv2.IMREAD_COLOR)
    # print(img.shape)
...
delta = np.array([depths[index][:,0]-depths[index-1][:,-1] for index in range(divide)])
...
output_directory = Path('output/'+folder+'/'+str(shift)+'/match_diff')
output_directory.mkdir(parents=True,exist_ok=True)
for index in range(len(depths_new)):
    np.save("output/{0}/{2}/match_diff/{0}-depth-{1}-{2}.npy".format(folder,index,shift),depths_new[index])
