...
for index in range(devide):
    depth = np.load("output/match/None/calib/"+folder+"-depth-"+str(index)+".npy")
    # depth = np.polyval(coeff,depth)
    img = cv2.imread("output/match/None/"+folder+"-"+str(index)+".png")
    ...
    delta = 2*math.pi*(index/devide+sift/(6*width))
    ...
    o3d.io.write_point_cloud(path_out+folder+"-match-"+str(index)+".pcd", pcd)
